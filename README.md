...
- ✅ **推送成功才标记已读**:只有推送成功的邮件才会打上已读标记;被过滤跳过的邮件与推送失败的邮件保持未读,下次轮询自动重试,不丢信
- 🔌 **连接管理**:每轮轮询结束自动登出 IMAP 连接,避免连接泄漏,多账户互不干扰
...
3. 按数量限制取前 N 封,逐封拉取内容(**暂不标记已读**)
4. 解析邮件:解码 MIME 头(主题/发件人)、提取正文(优先纯文本,回退 HTML)、解析日期(转换为北京时间)
5. 过滤:邮件日期早于 `BLOCK_BEFORE` 秒的旧邮件直接跳过;命中 `blocked_keywords` 屏蔽词的直接跳过——被过滤的邮件**保持未读**
6. 推送:调用飞书 Webhook 发送卡片消息,失败则回退纯文本
7. **推送成功**后才将该邮件标记为已读;推送失败则**保持未读**,下次轮询自动重试
8. 每封邮件之间休眠 `BATCH_DELAY` 秒,防止限流
9. 全部处理完后登出 IMAP 连接,休眠 `POLL_INTERVAL` 秒,进入下一轮
...
若希望所有邮件都保留未读状态,可删除 `lark-mail.py` 中 `monitor_account` 函数里的 `mail.uid('store', uid, '+FLAGS', '\\Seen')` 一行(约第 319 行)。
