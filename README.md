...
- **屏蔽词只匹配主题和正文**——要屏蔽某个发件人(如 "APS")请用 `blocked_senders` / `BLOCKED_SENDERS`,它匹配邮件 From 头(姓名和邮箱地址都会检查);屏蔽词为**子串匹配**,显示名含该子串即命中(如屏蔽 `芒果` 可命中 `芒果泥 <mangomud@example.com>`),但**纯拼音形态**(如 `mangguo`)不会命中,这种情况请改用完整邮箱地址
