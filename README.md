...
- 🚫 **关键词与发件人屏蔽**:支持按账户配置屏蔽词(命中主题或正文)与屏蔽发件人(命中 From 头),命中的邮件自动跳过,不推送;中英文逗号分隔,匹配不区分大小写
...
5. 双重过滤:搜索阶段已按时间限定;此处再校验邮件日期(个别邮件 Date 头异常时兜底);命中 `blocked_keywords` 屏蔽词(主题/正文)或 `blocked_senders` 屏蔽发件人(From 头)的直接跳过——被过滤的邮件**保持未读**
...
| `BLOCKED_KEYWORDS` | 否 | 空 | 屏蔽词,中英文逗号 `,`/`，` 分隔均可;匹配主题/正文,**不区分大小写** |
| `BLOCKED_SENDERS` | 否 | 空 | 屏蔽发件人,中英文逗号分隔;匹配 From 头(姓名/邮箱地址),**不区分大小写** |
...
[
  {
    ...
    "blocked_keywords": "广告,推广",             // 屏蔽词,中英文逗号分隔均可,匹配不区分大小写(可选)
    "blocked_senders": "APS,spam@x.com"         // 屏蔽发件人,中英文逗号分隔,匹配From头(可选)
  },
  {
...
- 检查是否被 `blocked_keywords` 屏蔽词误过滤(屏蔽词匹配不区分大小写,注意子串匹配——屏蔽词过短可能误伤,如 `aps` 也会命中 `maps`)

### 2.5 配置了屏蔽词/屏蔽发件人却没生效?

- **屏蔽词只匹配主题和正文**——要屏蔽某个发件人(如 "APS")请用 `blocked_senders` / `BLOCKED_SENDERS`,它匹配邮件 From 头(姓名和邮箱地址都会检查)
- **分隔符**:必须用逗号分隔,中英文逗号 `,`/`，` 均可(新版已兼容;旧版只认英文逗号)
- 查看启动日志 `屏蔽词: ...` / `屏蔽发件人: ...` 两行,确认分词结果与预期一致(日志显示的是原始大小写)
- 屏蔽词与发件人做**不区分大小写的子串匹配**,命中任一关键词即跳过推送
